...
def get_best_model(user_prompt) -> str:
    router_prompt = f"""사용자의 프롬프트/질문:
    <<<
    {user_prompt}
    >>>

    아래 규칙에 따라 가장 적합한 모델을 하나만 선택하세요.

    [강제 규칙 — 다음 표현이 하나라도 포함되면 반드시 gpt-5-thinking 선택]
    ...
    - gpt-4.1: 매우 긴 문맥을 읽고 요약/번역 등의 작업인 
    - gpt-4o: 일반적인 질문·대화·요약·번역 등 일상적 작업(기본값)
    - gpt-5-nano: 초고속·짧은 응답이 가장 중요한 경우

    [충돌 시 우선순위]
    - 복잡한 추론 신호가 조금이라도 있으면 gpt-5-thinking을 우선합니다
